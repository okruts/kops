...
func (c *UpgradeClusterCmd) Run(args []string) error {
	...
	channel := &api.Channel{}
	{
		// TODO: Cmd line arg?  Store in cluster?
		channelPath := "/home/justinsb/k8s/src/k8s.io/kops/channels/stable.yaml"
		channelBytes, err := vfs.Context.ReadFile(channelPath)
		if err != nil {
			return fmt.Errorf("error reading channel: %v", err)
		}
		err = api.ParseYaml(channelBytes, channel)
		if err != nil {
			return fmt.Errorf("error parsing channel: %v", err)
		}
